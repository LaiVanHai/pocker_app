module Pokers::CheckService
  def self.checkPockerHand(pocker_list)
  ...
  def self.validation(pocker_list, pocker_suit_hash)
    #　カードの数枚を確認する
    # msg = "5つのカード指定文字を半角スペース区切りで入力してください。（例：S1 H3 D9 C13 S11)"
    # return msg if pocker_list.count(" ") != (Settings.number_card - 1)
    # check_number_card_rgx = /^(([SHCD])(1[0-3]|0?[1-9])\s?){4}(([SHCD])(1[0-3]|0?[1-9]))$/m
    check_number_card_rgx = /^([a-zA-Z]\d+\s?){4}([a-zA-Z]\d+)$/m
    message = ""
    message += "5つのカード指定文字を半角スペース区切りで入力してください。（例：S1 H3 D9 C13 S11)" \
      unless pocker_list.match(check_number_card_rgx)
    ...
    pockers.each_with_index do |pocker, index|
      ...
      current_pocker_number = pocker.scan(number_regex)[0].to_i
  ...
  def self.checkAllCardTogetherWithSuit(suit_arr)
    Settings.suit_list.each do |suit|
  ...
  def self.checkSuit(pocker_list)
    check_suit_and_number_card_rgx = /^(([SHCD])(1[0-3]|0?[1-9])\s?){4}(([SHCD])(1[0-3]|0?[1-9]))$/m
    check_card_rgx = /^[SHCD]((1[0-3])|0?[1-9])$/
    #Pockerのスートと数を確認する
    respone_messages = ""

    unless pocker_list.match(check_suit_and_number_card_rgx)
      pockers = pocker_list.split(" ")
      pockers.each_with_index do |pocker, index|
        # unless Settings.suit_list.include?(pocker[0].upcase) &&
        #   [*"1".."13"].include?(pocker.slice(1..3))
        unless pocker.match(check_card_rgx)
  ...
  def self.checkArrayContinuous(array_number)
  ...
  def self.checkGroup(array_number)
